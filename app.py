...
#Notes
            ...
            html.Div([
                ...
                html.P("The default sexuality for all characters was straight. This decision was made because society is built around and for people who are straight. While sexuality should not be assumed, the social norm is to assume people are straight. Characters were only labeled as a member of the LGBTQ+ community if they discussed and/or identified their sexuality through a conversation or undoubtable on-screen relationship. While there are characters in the dataset who hint that they are not straight, if their sexuality has not been stated explicitly in some way, they are classified as straight."),
...
@app.callback(
    ...
    Input("int_panther","value"),
    Input("int_choice","value")
    )
def update_int_pie_chart(representation, blk_panthr, characters):
    char_list = ["Ayo", "Okoye", "Black Panther", "T'Challa", "Ramonda", "River Tribe Elder", "Dave (Henchman)", "Sope", "Sophia", "Erik Kilmonger", "Everett Ross", "T'Chaka", "Fiona Thomas", "Griot", "Ulysses Klaue", "Limbani", "Linda", "W'Kabi", "M'Baku", "M'Bele", "M'Kathu", "Merchant Tribe Elder", "Xoliswa", "Yama", "Zawavari", "Zuri", "N'Jobu", "Nakia", "Nomble", "Shuri"]
    df = pd.read_csv('Marvel Dataset.csv')
    if characters == 'Only Main Characters':
        df = df[df['Role'] == 'Main Character']
    if characters == 'Only Side Characters':
        df = df[df['Role'] == 'Side Character']
    if blk_panthr == ['Omit Black Panther Characters']:
        df = df[df['Name'].isin(char_list) == False]
...
def update_int_bar_graph(representation, blk_panthr, characters):
    char_list = ["Ayo", "Okoye", "Black Panther", "T'Challa", "Ramonda", "River Tribe Elder", "Dave (Henchman)", "Sope", "Sophia", "Erik Kilmonger", "Everett Ross", "T'Chaka", "Fiona Thomas", "Griot", "Ulysses Klaue", "Limbani", "Linda", "W'Kabi", "M'Baku", "M'Bele", "M'Kathu", "Merchant Tribe Elder", "Xoliswa", "Yama", "Zawavari", "Zuri", "N'Jobu", "Nakia", "Nomble", "Shuri"]
    df = pd.read_csv('Marvel Dataset.csv')
    if characters == 'Only Main Characters':
        df = df[df['Role'] == 'Main Character']
...
@app.callback(
    ...
    Input("rep_panther","value")
    )
def update_bar_chart(representation, characters, blk_panthr):
    char_list = ["Ayo", "Okoye", "Black Panther", "T'Challa", "Ramonda", "River Tribe Elder", "Dave (Henchman)", "Sope", "Sophia", "Erik Kilmonger", "Everett Ross", "T'Chaka", "Fiona Thomas", "Griot", "Ulysses Klaue", "Limbani", "Linda", "W'Kabi", "M'Baku", "M'Bele", "M'Kathu", "Merchant Tribe Elder", "Xoliswa", "Yama", "Zawavari", "Zuri", "N'Jobu", "Nakia", "Nomble", "Shuri"]
    df = pd.read_csv('Marvel Dataset.csv')
    if characters == 'Only Main Characters':
        df = df[df['Role'] == 'Main Character']
...
def update_time_graph(representation, characters, blk_panthr):
    f_total=0
    f_list = []
    s_total=0
    s_list = []
...
@app.callback(
    ...
    Input("mor_panther","value")
    )
def update_mor_bar_chart(representation, blk_panthr):
    df = pd.read_csv('Marvel Dataset.csv')
    char_list = ["Ayo", "Okoye", "Black Panther", "T'Challa", "Ramonda", "River Tribe Elder", "Dave (Henchman)", "Sope", "Sophia", "Erik Kilmonger", "Everett Ross", "T'Chaka", "Fiona Thomas", "Griot", "Ulysses Klaue", "Limbani", "Linda", "W'Kabi", "M'Baku", "M'Bele", "M'Kathu", "Merchant Tribe Elder", "Xoliswa", "Yama", "Zawavari", "Zuri", "N'Jobu", "Nakia", "Nomble", "Shuri"]
    if blk_panthr == ['Omit Black Panther Characters']:
        df = df[df['Name'].isin(char_list) == False]
...
def update_mor_pie_chart(representation, blk_panthr):
    df = pd.read_csv('Marvel Dataset.csv')
    char_list = ["Ayo", "Okoye", "Black Panther", "T'Challa", "Ramonda", "River Tribe Elder", "Dave (Henchman)", "Sope", "Sophia", "Erik Kilmonger", "Everett Ross", "T'Chaka", "Fiona Thomas", "Griot", "Ulysses Klaue", "Limbani", "Linda", "W'Kabi", "M'Baku", "M'Bele", "M'Kathu", "Merchant Tribe Elder", "Xoliswa", "Yama", "Zawavari", "Zuri", "N'Jobu", "Nakia", "Nomble", "Shuri"]
    if blk_panthr == ['Omit Black Panther Characters']:
        df = df[df['Name'].isin(char_list) == False]
